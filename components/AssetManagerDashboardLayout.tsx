...
export default function AssetManagerDashboardLayout({ children }: AssetManagerDashboardLayoutProps) {
  ...
  const navigationItems = [
    ...
    {
      path: '/asset-manager/dashboard/my-inspection',
      label: 'My Inspection',
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    ...
    {
      path: '/asset-manager/dashboard/inspection-status',
    ...
    {
      path: '/asset-manager/dashboard/delete-account',
    ...
    // {
    //   path: '/asset-manager/dashboard/reports',
    //   label: 'Reports',
    //   icon: (
    //     <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    ...
    // },
    // {
    //   path: '/asset-manager/dashboard/analytics',
    //   label: 'Analytics',
    //   icon: (
    //     <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div className="h-screen bg-[#E8F4F8] flex overflow-hidden">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 bg-gradient-to-b from-[#0D7FA8] to-[#0A5F7F] shadow-lg fixed h-full z-10">
        ...
        <div className="p-4 border-t border-[#0A5F7F]">
          <button
            onClick={() => router.push('/asset-manager/dashboard/settings')}
            className={`w-full flex items-center gap-3 px-4 py-3 mb-2 rounded-lg font-medium transition-colors ${isActive('/asset-manager/dashboard/settings') ? 'bg-white text-[#0D7FA8]' : 'text-white hover:bg-[#0A5F7F]'
              }`}
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-gradient-to-b from-[#0D7FA8] to-[#0A5F7F] shadow-lg z-50 transform transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        <div className="p-4 border-t border-[#0A5F7F]">
          <button
            onClick={() => {
              router.push('/asset-manager/dashboard/settings')
              setIsMobileMenuOpen(false)
            }}
            className={`w-full flex items-center gap-3 px-4 py-3 mb-2 rounded-lg font-medium transition-colors ${isActive('/asset-manager/dashboard/settings') ? 'bg-white text-[#0D7FA8]' : 'text-white hover:bg-[#0A5F7F]'
              }`}
        ...
        <header className="bg-gradient-to-r from-[#0D7FA8] to-[#0A5F7F] shadow-md px-4 md:px-6 py-4 sticky top-0 z-30">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 md:gap-4 ml-auto">
              <button
                onClick={() => router.push('/asset-manager/dashboard/notifications')}
