...
export default function ProfessionalServicesClient() {
  ...
  return (
    <main className="w-full min-h-screen bg-white overflow-x-hidden">
      ...
      <nav className="bg-[#E8F4F8] px-4 md:px-6 py-3 md:py-4"><div className="max-w-[1400px] mx-auto w-full flex items-center justify-between"><div className="hidden md:flex items-center gap-6 lg:gap-8"><Link href="/" className="text-sm font-medium text-gray-800 hover:text-[#0D6A8D]">HOME</Link><Link href="/about" className="text-sm font-medium text-gray-800 hover:text-[#0D6A8D]">ABOUT</Link><Link href="/inspection-services" className="text-sm font-medium text-gray-800 hover:text-[#0D6A8D]">SERVICES</Link><Link href="/professional-services" className="text-sm font-bold text-[#0D6A8D]">PROFESSIONALS</Link><Link href="/contact" className="text-sm font-medium text-gray-800 hover:text-[#0D6A8D]">CONTACT</Link></div><Button onClick={() => router.push("/profile-selection")} className="bg-[#1E88A8] hover:bg-[#176B87] text-white rounded-full px-6 py-2.5 text-sm font-medium shadow-md cursor-pointer">Login/Register</Button></div></nav>

      <section className="bg-black py-20 md:py-32 text-center relative overflow-hidden">
        <div className="absolute inset-0"><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#0D6A8D] rounded-full blur-[200px] opacity-20"></div></div>
        <div className="relative z-10"><p className="text-[#FF4757] font-bold uppercase tracking-[0.2em] mb-6">B2B Partnership Programs</p><h1 className="text-5xl md:text-7xl font-bold text-white mb-6">Professional Services</h1><p className="text-gray-400 text-xl max-w-3xl mx-auto">Tailored inspection partnerships for real estate professionals, property managers, developers, and financial institutions.</p></div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {partners.map((p, i) => (
            <div key={i} className="bg-white border border-gray-100 rounded-[32px] p-8 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group">
              ...
              <h3 className="text-xl font-bold text-black mb-4 group-hover:text-[#0D6A8D] transition-colors">{p.title}</h3>
      ...
      <section className="bg-[#0D6A8D] py-20 px-4 text-center"><h2 className="text-4xl font-bold text-white mb-6">Ready to Partner With Nspire?</h2><p className="text-white/70 text-lg mb-10 max-w-2xl mx-auto">Join our network of professional partners and deliver best-in-class inspection services to your clients.</p><Button onClick={() => router.push("/contact")} className="bg-[#FF4757] hover:bg-[#EE3646] text-white rounded-full px-12 py-7 text-lg font-bold shadow-2xl hover:scale-105 transition-all">Become a Partner</Button></section>
