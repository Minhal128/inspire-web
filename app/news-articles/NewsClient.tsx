...
export default function NewsClient() {
  ...
  return (
    <main className="w-full min-h-screen bg-white overflow-x-hidden">
      ...
      <section className="bg-[#0D6A8D] py-16 md:py-24 text-center"><h1 className="text-5xl md:text-7xl font-bold text-white mb-4">News & Articles</h1><p className="text-white/70 text-lg">Stay informed with the latest from Nspire and the inspection industry.</p></section>
      ...
      <section className="max-w-[1000px] mx-auto px-4 md:px-6 py-16 md:py-24 space-y-10">
        {articles.map((a, i) => (
          <article key={i} className="border-b border-gray-100 pb-10 last:border-0">
            <div className="flex items-center gap-3 mb-4"><span className="bg-[#E8F4F8] text-[#0D6A8D] px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{a.category}</span><span className="text-gray-400 text-sm">{a.date}</span></div>
            <h2 className="text-2xl md:text-3xl font-bold text-black mb-4 hover:text-[#0D6A8D] transition-colors cursor-pointer">{a.title}</h2>
