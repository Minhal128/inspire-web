...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
    ...
    const data = stateData[state.toLowerCase()];
    if (!data) return { title: "Not Found" };
    return {
        title: `Certified Home Inspectors in ${data.name} | NSPIREinspection.AI`,
...
export default async function StateInspectorsPage({ params }: { params: Promise<{ state: string }> }) {
    const { state } = await params;
    const data = stateData[state.toLowerCase()];
    if (!data) notFound();
